...
print join("\t", "Gene", "Transcript/CCDS", 'Exp depth', 'Reads for 20x', 'bp missing', '% missing', 'bp 1-5x', '% 1-5x', 'bp 6-9x', '% 5-9x', 'bp 10-19x', '% 10-19x') . "\n";
...
#    next if ( $$exon{ exon_name } ne "APC_Exon15");
    ...
    foreach my $coverage ( @coverages ) {
      ...
      next if ( $$sample_sequence_hash{ total_reads } < 20_000_000);
...
#	print "-------->>> $meanX, $meanY -- $exp_depth_30M\n";

	my $reads_for_20x = 0;
	$reads_for_20x    = 20/$meanY*30_000_000 if ( $meanY );

#	print join("\t", $exp_depth_30M, $reads_for_20x/1_000_000) . "\n";
...
#	print "-------->>> $meanX1to5, $meanY1to5 -- $exp_1to5_30M\n";
...
#	print "-------->>> $meanX10to19, $meanY10to19 -- $exp_10to19_30M\n";


	$transcripts{ "$refseq/$CCDS" }{'length'}  += $$region{end} - $$region{start} + 1;
	$transcripts{ "$refseq/$CCDS" }{'missing'} += $exp_missing_30M;
	$transcripts{ "$refseq/$CCDS" }{'1to5'}    += $exp_1to5_30M;
	$transcripts{ "$refseq/$CCDS" }{'6to9'}    += $exp_6to9_30M;
	$transcripts{ "$refseq/$CCDS" }{'10to19'}  += $exp_10to19_30M;

	if ( ! $transcripts{ "$refseq/$CCDS" }{'min'} || ($transcripts{ "$refseq/$CCDS" }{'min'}  > $exp_depth_30M) ) {
	  $transcripts{ "$refseq/$CCDS" }{'min'}  = $exp_depth_30M;
	}

	if ( ! $transcripts{ "$refseq/$CCDS" }{'20x'} || ($transcripts{ "$refseq/$CCDS" }{'20x'}  < $reads_for_20x) ) {
	  $transcripts{ "$refseq/$CCDS" }{'20x'}  = $reads_for_20x;
	}
...
#	printf("$gene_name\t$refseq\t$CCDS\t$$region{chr}:$$region{start}-$$region{end}\t$$exon{exon_name}\t%.2f\t%dM\t$exp_missing_30M\t$exp_1to5_30M\t$exp_6to9_30M\t$exp_10to19_30M\n", $exp_depth_30M, $reads_for_20x/1_000_000, );
      }
...
#  exit;
  ...
  foreach my $transcript (keys %transcripts ) {
    ...
    printf("$gene_name\t$transcript\t%.1fx\t%.1fM\t%.2f\t%.2f%%\t%.2f\t%.2f%%\t%.2f\t%.2f%%\t%.2f\t%.2f%%\n", 
	   $transcripts{ $transcript }{'min'}, $transcripts{ $transcript }{'20x'}/1_000_000,
	   $transcripts{ $transcript }{ 'missing' }, 100*$transcripts{ $transcript }{ 'missing' }/$transcripts{ $transcript }{ 'length' }, 
	   $transcripts{ $transcript }{ '1to5' }, 100*$transcripts{ $transcript }{ '1to5' }/$transcripts{ $transcript }{ 'length' }, 
	   $transcripts{ $transcript }{ '6to9' }, 100*$transcripts{ $transcript }{ '6to9' }/$transcripts{ $transcript }{ 'length' }, 
	   $transcripts{ $transcript }{ '10to19' }, 100*$transcripts{ $transcript }{ '10to19' }/$transcripts{ $transcript }{ 'length' });
  }
