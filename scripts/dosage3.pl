...
#  next if ( $$exon{ exon_name} ne "FBN1_Exon63" && $$exon{ exon_name} ne "FBN1_Exon64");
...
#    print "$$sample_sequence_hash{ name }\n";
    ...
    if ( $$sample_sequence_hash{ name } eq $sample_name ) {
      $sample_min_depth = $$coverage{min} || 0;
...
#  print Dumper( $region );
  ...
  if (! @min_depth) {
    ...
    for( my $i = 0; $i < @min_depth; $i++ ) {

      my ( $depth, $reads ) = ( $min_depth[ $i ], $reads[ $i ]);
...
#      print  " -- $ratio \n";
    ...
    my $avg_ratio = $summed_ratios/$ratios;
    $avg_ratio = sprintf("%.2f", $avg_ratio);
